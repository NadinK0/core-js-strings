...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) !== -1) {
    ...
    const str2 = str.substring(str.indexOf(value) + value.length, str.length);
...
function removeLastOccurrences(str, value) {
  ...
  if (str.lastIndexOf(value) !== -1) {
    const b = str.lastIndexOf(value) + value.length;
    const str1 = str.substring(0, str.lastIndexOf(value));
    const str2 = str.substring(b, str.length);
    newStr = str1.concat(str2);
  }
  return newStr;
}
...
function formatTime(minutes, seconds) {
  ...
  return str1.concat(seconds.toString().padStart(2, '0'));
}
...
function findLongestWord(sentence) {
  ...
  const maxIndex = newArr.indexOf(Math.max.apply(null, newArr));
  return arr[maxIndex];
}
...
function reverseWords(str) {
  ...
  const newArr = arr.map((e) => e.reverse().join(''));
  return newArr.join(' ');
}
